...
final class ExplicitJoinStrategy extends SelectOnlyJoinStrategy
{
    ...
    private function assertColumnsAreInSelectList(SelectCommon $select, array $joinedColumns, string $alias): array
    {
        ...
        // We don't use a walker as we only need to check the top level list
        $columnsHash = [];
        try {
            // We have a table with an assigned alias at the top(-ish) level?
            $this->findNodeForJoin($select->from, $alias);
            // What's being joined was probably generated by an instance of TableSelect
            ...
            // What's being joined is probably a manually written query with custom table aliases, and it is probably
            // expected by its author to end up in a subselect
            foreach ($select->list as $item) {
                if ($item instanceof Star) {
                    return [];
                }
                if (null !== $item->alias) {
                    $columnsHash[$item->alias->value] = null;
                } elseif ($item->expression instanceof ColumnReference) {
                    if ($item->expression->column instanceof Star) {
                        return [];
                    }
                    $columnsHash[$item->expression->column->value] = null;
                }
            }
        }
